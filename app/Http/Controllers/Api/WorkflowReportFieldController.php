...
class WorkflowReportFieldController extends Controller
{
    ...
    public function index(int $idWorkflow, Request $request)
    {
        if (isset($request->stage_id)) {
            if (isset($request->task_id)) {
                $reports = Field::query()->where('model', 'report-field')->where('workflow_id', $idWorkflow)->where('stage_id', $request->stage_id)->get();
            ...
            $reports = Field::query()->where('model', 'report-field')->where('workflow_id', $idWorkflow)->get();
        }
